...
if [ "$running_sequence" == "h1" ]; then
	scenes=$(echo $scenes | cut -d' ' -f1-9)
	scnen_paths=$(echo $scnen_paths | cut -d' ' -f1-9)
elif [ "$running_sequence" == "h2" ]; then
	scenes=$(echo $scenes | cut -d' ' -f10-17)
	scnen_paths=$(echo $scnen_paths | cut -d' ' -f10-17)
elif [ "$running_sequence" == "id" ]; then
	read -p "Enter the index of the scene (0~16, default: 0): " seq_idx
	if [ -z "$seq_idx" ]; then
		seq_idx=0
	fi
