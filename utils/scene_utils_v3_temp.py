...
def render_training_image_v3(scene, gaussians_static, gaussians_dynamic, viewpoints, render_func, pipe, background, stage, iteration, time_now, dataset_type, gvc_params):
    def render(gvc_params, gaussians_static, gaussians_dynamic,  viewpoint, path, scaling, cam_type):
        # scaling_copy = gaussians._scaling
        render_pkg = render_func(gvc_params, viewpoint, gaussians_static, gaussians_dynamic, pipe, background, stage=stage, cam_type=cam_type)
        label1 = f"stage:{stage},iter:{iteration}"
        times =  time_now/60
        if times < 1:
            end = "min"
    ...
    for idx in range(len(viewpoints)):
        image_save_path = os.path.join(image_path,f"{iteration}_{idx}.jpg")
        render(gvc_params, gaussians_static, gaussians_dynamic, viewpoints[idx],image_save_path,scaling = 1,cam_type=dataset_type)
    pc_mask = gaussians_dynamic.get_opacity
    pc_mask = pc_mask > 0.1
