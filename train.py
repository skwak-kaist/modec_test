...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    for iteration in range(first_iter, final_iter+1):        
        ...
        if opt.lambda_dssim != 0:
            # balancing between dssim and l1 loss
        ...
        if stage == "fine":
            if hyper.time_smoothness_weight != 0:
                tv_loss = gaussians.compute_regulation(hyper.time_smoothness_weight_global, hyper.l1_time_planes_global, hyper.plane_tv_weight_global, 
                                                       hyper.time_smoothness_weight, hyper.l1_time_planes, hyper.plane_tv_weight)
                loss += tv_loss
            if hyper.anchor_dynamics_position or hyper.anchor_dynamics_local_context_feature: # if anchor dynamics is activated
                ...
                if (iteration < 1000 and iteration % 10 == 9) \
                    or (iteration < 3000 and iteration % 50 == 49) \
                        ...
                        render_training_image(scene, gaussians, [test_cams[iteration%len(test_cams)]], render, pipe, background, 
                                                stage+"test", iteration,timer.get_elapsed_time(),scene.dataset_type)
                        render_training_image(scene, gaussians, [train_cams[iteration%len(train_cams)]], render, pipe, background, 
                                                stage+"train", iteration,timer.get_elapsed_time(),scene.dataset_type)

            timer.start()
            
            # Densification (anchor adjustment process)
            if iteration < opt.update_until and iteration > opt.start_stat:
                # add statis
                gaussians.training_statis(viewspace_point_tensor, opacity, visibility_filter, offset_selection_mask, voxel_visible_mask)
                # densification (anchor)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[1000, 14000, 20000, 30_000, 45000, 60000])
